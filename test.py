...
canvas.pack()
bat = canvas.create_rectangle(0, 0, 40, 10, fill="dark green")
...
def check_game_over():
    ...
    if ballTop > canvasHeight:
        print(" Ton score est de " + str(score)+" rebonds ")
        playAgain = tkinter.messagebox.askyesno(message="Une nouvelle partie ?")
        if playAgain == True:
            reset()
        else:
            close()
